#' A parse for extracting contents of `c()` in a character vector representation.
#'
#' @param vec_str The character vector (string), e.g., `c(value1, value2, value3)`
#'
#' @return Returns a parsed, unvectorised character vector.
#' @examples
#'
#' LexOPS:::parse_unvectorise("c(hello, world)")

parse_unvectorise <- function(vec_str) {
  if (grepl("^c\\(.+\\)$", vec_str)) {
...
#' A non-standard evaluation parser for levels (e.g., for `split_by()`)
#'
...
#'
#' This is useful for specifying multiple parameters in non-standard evaluation for `match_item()`.
#'
