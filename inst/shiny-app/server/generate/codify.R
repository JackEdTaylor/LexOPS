...
output$gen_codify_text <- renderText({
  ...
  seed_set <- if (is.na(gen_seed)) "" else sprintf("set.seed(%i)\n\n", gen_seed)
  out <- sprintf("library(LexOPS)\n\n%sstim <- LexOPS::lexops", seed_set)
  ...
  if (gen_splitby_boxes_N() > 0) {
    for (i in 1:gen_splitby_boxes_N()) {
      if (split_opts[[i]]$var == "Random") {
        out <- sprintf("%s |>\n\tsplit_random(%i, equal_size=TRUE)", out, codify_var(split_opts[[i]]$n_levels))
      } else {
