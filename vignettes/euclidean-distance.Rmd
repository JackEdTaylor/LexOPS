...
Euclidean distance is the straight-line distance between two points in $n$-dimensional space. Between points $a$ and $b$, the Euclidean distance $d(a,b)$ calculated as:
...
This makes our 2-dimensional space look like the plot below. The dashed lines now reflect the values that we would from the LexOPS function `euc_dists()`, which scales dimensions by default.
...
As an example, if we decide to give valence twice the weight of concreteness, our 2D space would look like this:
...
Alternatively, we can set weights such that frequency accounts for more of Euclidean distance than other variables, with the `weights` argument:
...
This generate stimuli with the following distributions in controls:
...
This will generate stimuli with the following distributions in controls. As you can see, we've generated stimuli that are closely matched in length and frequency, and more loosely matched in Age of Acquisition. The stimuli occasionally vary slightly in length in cases when age of acquisition and frequency are very similar:
