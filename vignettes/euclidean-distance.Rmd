...
The stimuli occasionally vary slightly in length in cases when age of acquisition and frequency distances are small. If we want to use this same Euclidean space, but limit how much the variables are allowed to differ, we could combine `control_for_euc()` with `control_for()`. Here we make sure that words do not differ by more than ±1 character in Length.
